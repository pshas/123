...
Ниже подробно покажу, как по-шагам организовать «создание блока» через инфоблок, чтобы после этого в вашем шаблоне `news.list` появлялись добавленные элементы (а не «Блоки не найдены»).

---

## 1. Создание инфоблока «Блоки лаборатории»

Прежде чем писать форму, нужно убедиться, что сам инфоблок существует и мы знаем его `ID`.

1. **Перейдите в админку**: «Контент → Инфоблоки (Администрирование инфоблоков) → Типы инфоблоков».

   * Если у вас ещё нет собственного типа, создайте его (например, с кодом `lab`).

2. **Затем зайдите в «Контент → Инфоблоки»**, выберите нужный тип `lab` и нажмите «Добавить инфоблок».

   * Название: «Блоки лаборатории»
   * Символьный код: `lab_blocks` (или любой другой, но запомните его).
   * Активность: «Да»
   * Остальные поля можете оставить по умолчанию.

3. После сохранения зайдите внутрь созданного инфоблока, откройте «Список элементов» и обратите внимание на его **ID** (он отображается в заголовке над списком или в URL при редактировании). Предположим, что это `10`.

4. (Опционально) Зайдите во вкладку «Свойства» и создайте пару полей, если они вам нужны.
   Например:

   * Свойство «Описание» (код `DESCRIPTION`), тип «Строка (textarea)» или «HTML/текст».
   * Свойство «Картинка» (код `PICTURE`), тип «Файл».
   * Или любые другие свойства, которые нужны для «Блока».

После этого у вас должен быть работающий инфоблок с ID = `10` (или другой, который вы увидите).

---

## 2. Подключаем компонент «форма добавления элемента»

Теперь создадим страницу, откуда будет происходить добавление «блока». Предположим, это `/local/glab/add_block.php`.

1. **Создайте файл** `/local/glab/add_block.php` с таким содержимым:

   ```php
   <?php
   // 1) Запускаем движок Битрикса
   require_once $_SERVER['DOCUMENT_ROOT'].'/header.php';
   $APPLICATION->SetTitle("Добавить новый блок");

   // 2) Подключаем компонент формы
   $APPLICATION->IncludeComponent(
       "bitrix:iblock.element.add.form",
       "", // пустой шаблон, будет стандартный
       [
           "IBLOCK_TYPE"           => "lab",         // ваш тип инфоблока
           "IBLOCK_ID"             => "10",          // ID инфоблока «Блоки лаборатории»
           "ELEMENT_ID"            => "",            // пусто, т.е. добавление, а не редактирование
           "ELEMENT_CODE"          => "",
           "PROPERTY_CODES"        => [
               "NAME",               // Заголовок элемента
               "DETAIL_TEXT",        // Текст (если вы хотите использовать стандартное поле Detail/Text)
               "PROPERTY_DESCRIPTION", // свойство «Описание», если создали его
               "PROPERTY_PICTURE",     // свойство «Картинка», если есть
               // Добавьте здесь коды тех свойств, которые хотите редактировать
           ],
           "PROPERTY_CODES_REQUIRED" => ["NAME"],      // минимум — обязательное поле «Название»
           "GROUPS"                => ["*"],          // группы, которым разрешено добавлять (звездочка — всем авторизованным)
           "STATUS_NEW"            => "N",            // статус сразу активировать
           "STATUS"                => "ANY",          // не важно, какой статус (компонент установит ACTIVE=Y)
           "USE_CAPTCHA"           => "N",
           "USER_MESSAGE_ADD"      => "Блок успешно добавлен.",
           "USER_MESSAGE_EDIT"     => "Данные обновлены.",
           "DEFAULT_INPUT_SIZE"    => "30",
           "RESIZE_IMAGES"         => "N",
           "MAX_FILE_SIZE"         => "0",
           ...
           "AJAX_OPTION_STYLE"     => "Y",
           "AJAX_OPTION_HISTORY"   => "N",
           // Названия полей, которые будет видеть пользователь
           "CUSTOM_TITLE_NAME"     => "Название блока",
           "CUSTOM_TITLE_PREVIEW_TEXT" => "Краткое описание",
           "CUSTOM_TITLE_PROPERTY_DESCRIPTION" => "Детальное описание",
           "CUSTOM_TITLE_PROPERTY_PICTURE"     => "Изображение блока",
       ],
       false
   );
   ...
   * `"IBLOCK_TYPE" => "lab"` и `"IBLOCK_ID" => "10"` замените на ваши значения из админки.
   * В `PROPERTY_CODES` перечислены коды свойств, которые вы создали (например, `PROPERTY_DESCRIPTION` или `PROPERTY_PICTURE`).
   * `GROUPS => ["*"]` означает, что форму могут заполнять все **авторизованные** пользователи. Если вы хотите, чтобы добавлять могли и гости, замените на `["2"]` (группа всех пользователей).

2. **Зайдите в браузере** на `https://ваш-домен/local/glab/add_block.php`. Должна отобразиться форма:

   * Поле «Название блока» (которое будет сохраняться в `NAME`).
   * Поле «Краткое описание» (mapped на `PREVIEW_TEXT`).
   * Поля для каждого `PROPERTY_*`, которые вы указали: «Детальное описание», «Изображение блока» и т. д.
   * Кнопка «Добавить» и «Отмена» (стандартная).

3. **Заполните форму и нажмите «Добавить»**.

   Если всё настроено верно, компонент создаст в инфоблоке новый элемент и выведет текст «Блок успешно добавлен.» (из параметра `USER_MESSAGE_ADD`).

---

## 3. Проверка: правим `/local/glab/index.php` и «Блоки не найдены»

После того как вы хотя бы один блок добавили, зайдите снова на страницу, где у вас стоит вызов `news.list` (например, `/local/glab/index.php`). Убедитесь, что там в параметрах `IncludeComponent` вы указали правильный `IBLOCK_ID => "10"`, а также правильно настроили `PROPERTY_CODE` и `FIELD_CODE` (хотя бы `"NAME"` и `"PREVIEW_TEXT"`).

Пример нужного кода в `/local/glab/index.php`:

```php
<?php
require_once $_SERVER['DOCUMENT_ROOT'].'/header.php';
$APPLICATION->SetTitle("Список блоков");

// Подключаем вывод списка
$APPLICATION->IncludeComponent(
    "bitrix:news.list",
    "lab_blocks_template", // шаблон, который мы создавали ранее
    [
        "IBLOCK_TYPE"            => "lab",
        "IBLOCK_ID"              => "10",           // обратите внимание на ID
        "NEWS_COUNT"             => "50",
        "SORT_BY1"               => "ID",
        "SORT_ORDER1"            => "DESC",
        "FIELD_CODE"             => ["ID", "NAME", "PREVIEW_TEXT", "PREVIEW_PICTURE"],
        "PROPERTY_CODE"          => ["DESCRIPTION", "PICTURE"], // если есть такие свойства
        "CHECK_DATES"            => "Y",
        "DETAIL_URL"             => "",
        "AJAX_MODE"              => "N",
        "CACHE_TYPE"             => "A",
...
Если у вас всё ещё «Блоки не найдены», проверьте:

1. **Правильно ли указан `IBLOCK_ID`** – тот самый, в котором вы сейчас создавали элементы.
2. Существует ли хоть один **активный** элемент (`ACTIVE = Y`) в этом инфоблоке (перейдите в админке «Список элементов» и убедитесь, что в списке есть записи).
3. Убедитесь, что вы не фильтруете по дате (параметр `"CHECK_DATES" => "Y"` не скрывает ли ваши элементы из-за неподходящей даты «активности»).

---

## 4. (Опционально) Кастомная JS-логика для «Оформить»

В шаблоне `lab_blocks_template/template.php` у каждой карточки блока была форма:
...
Вместо простой отправки формы через перезагрузку страницы, вы можете сделать AJAX-запрос (см. пример в предыдущем ответе). Либо оставить как есть — это дело вкуса.
      ...
      /components
        /bitrix
          /news.list
            /lab_blocks_template
              template.php
              style.css       ← стили вывода списка блоков
      header.php
      footer.php
/local
  /glab
    add_block.php         ← форма добавления нового блока
    index.php             ← здесь вывод списка (news.list)
    order.php             ← скрипт «оформить» (создание заявки)
    .top.menu.php         ← (ваш код меню, если нужен)
/header.php             ← корневой, подключает движок
/footer.php             ← корневой, завершает вывод
```

1. **`add_block.php`** – отвечает за **добавление** нового элемента (блока)
2. **`index.php`** – выводит список блоков через `news.list / lab_blocks_template`
3. **`order.php`** – обрабатывает «Оформить» (создание заявки)
4. Шаблон `lab` (`/bitrix/templates/lab/`) содержит самописный компонент меню и стили

---

После того как вы выполните все шаги и убедитесь, что в инфоблоке есть хотя бы один элемент, на `/local/glab/index.php` больше не будет надписи «Блоки не найдены»: появятся карточки ваших блоков. Если при добавлении через `add_block.php` всё прошло успешно, то сразу после редиректа (либо при переходе на `/local/glab/index.php`) вы увидите новую запись.

Если после этого останутся вопросы о тонкостях оформления или обработке «Оформить», дайте знать — разберём пример AJAX-реализации или дополнительные свойства.
